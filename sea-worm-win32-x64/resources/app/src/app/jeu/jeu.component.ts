...
export class JeuComponent implements OnInit {
  ...
  getPoisonPosition(): Case {
    ...
    do {
      ...
      for (const pixel of this.seaWorm.getCases()) {
        ...
        if (
          idToCheck == this.poison1.getCase().getId() ||
  ...
  checkBites(): boolean {
    if (this.seaWorm.getCases().length >= 2) {
      ...
      for (const pixel of restOfBody) {
        if (
          pixel.getPositionX() == head.getPositionX() &&
          pixel.getPositionY() == head.getPositionY()
        ) {
          return true;
        }
      }
      if (
        (head.getId() == this.poison1.getCase().getId() && this.poison1.getToShow()) ||
  ...
  extraShrinkWorm(): void {
    const wormShrinked = this.seaWorm.getCases().slice(0, 1);
  ...
  mediumShrinkWorm(): void {
    if (this.seaWorm.getCases().length > 6) {
      const wormShrinked = this.seaWorm.getCases().slice(0, this.seaWorm.getCases().length - 3);
      this.seaWorm.setCases(wormShrinked);
    }
    this.food.setType(TYPE.yellowgreen);
  }
  ...
  shrinkWorm(): void {
    if (this.seaWorm.getCases().length > 4) {
      const wormShrinked = this.seaWorm.getCases().slice(0, this.seaWorm.getCases().length - 2);
      this.seaWorm.setCases(wormShrinked);
    }
    this.food.setType(TYPE.yellowgreen);
  }
  ...
  growWorm(): void {
    ...
    if (this.seaWorm.getCases().length > 1) {
      ...
      pixelToAdd = new Case(
        State.worm,
        lastPixelWorm.getPositionX() -
          (beforeLastPixelWorm.getPositionX() - lastPixelWorm.getPositionX()),
        lastPixelWorm.getPositionY() -
          (beforeLastPixelWorm.getPositionY() - lastPixelWorm.getPositionY())
      );
  ...
  showPixels(worm: Worm): void {
    ...
    for (const pixel of this.grid) {
      ...
      if (
        (this.poison1 != undefined &&
          this.poison2 != undefined &&
          this.poison3 != undefined &&
          pixel.getId() == this.poison1.getCase().getId() &&
          this.poison1.getToShow()) ||
  ...
  storeScore(): void {
    this.gameService.saveRecord(
      this.food.getCount(),
      this.gameService.getCode1(),
      this.gameService.getCode2()
    );
  }
